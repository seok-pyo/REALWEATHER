...
- API 보안, CORS 해결하기 위해 vercel serverless 도입

고민10.

- 검색된 지역들을 선택하는 방식. 클로저
- onClick({handlePlace(item)}) / onClick({() => handlePlace(item)})
...
- 시간별 날씨 위젯에서 가로로 스크롤을 넘길 때 '날짜'를 표시하는 방법
...
- 메시지를 보여주는 부분에서 삼항 연산자를 이용하여, 데이터의 상태에 따른 메시지를 보여줄 수 있도록 구현.
